...
#define REG_GPIOG_MFPL 		( 0xF8002000+0x0A0)              
#define REG_GPIOG_MFPH 		( 0xF8002000+0x0A4)   

#define REG_GPIOG_DATAOUT       ( 0xF8003000+ 0x184)
#define REG_GPIOG_DATAIN        ( 0xF8003000+ 0x188)
#define REG_GPIOG_DIR           ( 0xF8003000+ 0x180)

//IO操作函数	 
#define SET_IIC_SCL    *(volatile unsigned int *)REG_GPIOG_DATAOUT|=1 //SCL
#define SET_IIC_SDA    *(volatile unsigned int *)REG_GPIOG_DATAOUT|=1<<1 //SDA	

#define CLE_IIC_SCL    *(volatile unsigned int *)REG_GPIOG_DATAOUT&=~(1) //SCL
#define CLE_IIC_SDA    *(volatile unsigned int *)REG_GPIOG_DATAOUT&=~(1<<1) //SDA	


#define READ_SDA       ( *(volatile unsigned int *)REG_GPIOG_DATAIN&(1<<1) )//输入SDA 
...
void IIC_Init(void)
{					     
   *(volatile unsigned int *)REG_GPIOG_MFPL&=~(0x00FF);
   *(volatile unsigned int *)REG_GPIOG_DIR|=0x3;
   *(volatile unsigned int *)REG_GPIOG_DATAOUT|=0x3;
   delay_ms(10);
}
...
void SDA_IN()
{
    *(volatile unsigned int *)REG_GPIOG_DIR&=~(1<<1);
}
void SDA_OUT()
{
    *(volatile unsigned int *)REG_GPIOG_DIR|=1<<1;
...
u8 I2C_WriteNAU8822(uint8_t addr, uint16_t data)
{
			...
			struct i2c_msg *msgs, int num)
{
	...
	for (retry = 0; retry < adap->retries; retry++) {
		...
		ret=I2C_WriteNAU8822(msgs->buf[0],msgs->buf[1]);
	        
		if(ret) printk("NAU8822 on find \n");
