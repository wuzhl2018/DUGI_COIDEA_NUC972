...
export MODNAME=iproute2

export MODPKG_NAME=iproute2-3.10.0.tar.gz
export MODSRC_NAME=iproute2-3.10.0
...
{
	cd ${MODSRC_PATH}
    ./configure --host=${HOST} --prefix=${MODOUT_PATH}
    [ $? -ne 0 ] && perro "Config module Failed!" && exit 1
	pdone "Config module OK!"
	make
    [ $? -ne 0 ] && perro "Build module Failed!" && exit 1
	pdone "Build module OK!"
	if [ ! -d ${MODOUT_PATH}/usr/local/bin ]; then
		mkdir -p ${MODOUT_PATH}/usr/local/bin
	fi
	cp ip/ip ${MODOUT_PATH}/usr/local/bin/
...
if [ "x$1" = "xbuild" ]; then
	...
	export LDFLAGS="-L${MODOUT_PATH}/usr/lib"
