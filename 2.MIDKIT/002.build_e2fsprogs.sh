...
export MODNAME=e2fsprogs

export MODPKG_NAME=e2fsprogs-1.42.9.tar.gz
export MODSRC_NAME=e2fsprogs-1.42.9
...
{
	cd ${MODSRC_PATH}
    ./configure  --host=${HOST} --prefix=${MODOUT_PATH} --enable-elf-shlibs
	make && make install
    [ $? -ne 0 ] && perro "Build module Failed!" && exit 1
	mkdir -p ${MODOUT_PATH}/include/uuid
	mkdir -p ${MODOUT_PATH}/include/blkid
	cp ${MODSRC_PATH}/lib/uuid/uuid.h ${MODOUT_PATH}/include/uuid/
	cp ${MODSRC_PATH}/lib/blkid/*.h   ${MODOUT_PATH}/include/blkid/
