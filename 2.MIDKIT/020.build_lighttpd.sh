...
export MODNAME=lighttpd

export MODPKG_NAME=lighttpd-1.4.39.tar.gz
export MODSRC_NAME=lighttpd-1.4.39
...
{
	cd ${MODSRC_PATH}
    ./configure --prefix=${MODOUT_PATH} --host=${HOST} --without-zlib --with-pcre --without-bzip2
	make && make install
...
if [ "x$1" = "xbuild" ]; then
	...
	export PATH=${PATH}:${MODOUT_PATH}/bin
